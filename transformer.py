...
from keras_transformer.keras_transformer.attention import MultiHeadSelfAttention
...
from keras import layers  #  To access various layers for neural network construction
...
class TransformerBlock:
    ...
    def __call__(self, _input):
        ...
        h = int(tf.math.sqrt(tf.shape(output)[1]))
        c = tf.shape(output)[-1]
        x = tf.reshape(output, (-1, h, h, c))

        x = channel_attention(x)

        # reshape back → token sequence
        output = tf.reshape(x, (-1, h * h, c))
        post_residual2 = (
