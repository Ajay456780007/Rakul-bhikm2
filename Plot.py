...
class ALL_GRAPH_PLOT:
    ...
    def load_kf_perf_values(self, DB):
        # Concat_epochs(DB)
        A11 = np.load(f"Analysis/KF_Perf/{DB}/kf_metrics_epochs_100.npy")[:8, :] * 100
        A22 = np.load(f"Analysis/KF_Perf/{DB}/kf_metrics_epochs_200.npy")[:8, :] * 100
        A33 = np.load(f"Analysis/KF_Perf/{DB}/kf_metrics_epochs_300.npy")[:8, :] * 100
        A44 = np.load(f"Analysis/KF_Perf/{DB}/kf_metrics_epochs_400.npy")[:8, :] * 100
        A55 = np.load(f"Analysis/KF_Perf/{DB}/kf_metrics_epochs_500.npy")[:8, :] * 100
    ...
    def KF_perf_figure(self, DB, perf, x_label, y_label, colors, bar_width):
        n_epochs = perf.shape[0]
        n_percentage = perf.shape[1]
        Model = self.models2
        Models = Model[:n_epochs]
        ...
        if self.save:
            os.makedirs(f"Results/{DB}/KF_PERF/Bar", exist_ok=True)
            df.to_csv(f"Results/{DB}/KF_PERF/Bar/{y_label}__Graph.csv")
            plt.savefig(f"Results/{DB}/KF_PERF/Bar/{y_label}_Graph.png", dpi=600)
            print(colored(f'KF_PERF Bar Graph Image for {DB} of {y_label} saved', 'green'))
    ...
    def KF_perf_figure_line(self, DB, perf, x_label, y_label, colors):
        n_epochs = perf.shape[0]
        n_percentage = perf.shape[1]
        Model = self.models2
        Models = Model[:n_epochs]
        ...
        if self.save:
            os.makedirs(f"Results/{DB}/KF_PERF/Line", exist_ok=True)
            df.to_csv(f"Results/{DB}/KF_PERF/Line/{y_label}__Graph.csv")
            plt.savefig(f"Results/{DB}/KF_PERF/Line/{y_label}_Graph.png", dpi=600)
            print(colored(f'KF_PERF Line Graph Image for {DB} of {y_label} saved', 'green'))
    ...
    def KF_plot_perf_figure(self, DB):
        data = self.load_kf_perf_values(DB)
        ...
        Perf_2 = np.sort(Perf_2.T).T
        Perf_2 = np.sort(Perf_2)
        self.KF_perf_figure(DB, Perf_2, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_2, x_label, y_label, self.color)

        y_label = "Recall (%)"
        Perf_3 = data[4]
        Perf_3 = np.sort(Perf_3.T).T
        Perf_3 = np.sort(Perf_3)
        self.KF_perf_figure(DB, Perf_3, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_3, x_label, y_label, self.color)

        y_label = "Accuracy (%)"
        Perf_1 = data[0]
        Perf_1 = np.sort(Perf_1.T).T
        Perf_1 = np.sort(Perf_1)
        self.KF_perf_figure(DB, Perf_1, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_1, x_label, y_label, self.color)

        y_label = "F1 Score (%)"
        Perf_4 = data[3]
        Perf_4 = np.sort(Perf_4.T).T
        Perf_4 = np.sort(Perf_4)
        self.KF_perf_figure(DB, Perf_4, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_4, x_label, y_label, self.color)
        ...
        Perf_5 = np.sort(Perf_5.T).T
        Perf_5 = np.sort(Perf_5)
        self.KF_perf_figure(DB, Perf_5, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_5, x_label, y_label, self.color)
        ...
        Perf_6 = np.sort(Perf_6.T).T
        Perf_6 = np.sort(Perf_6)
        self.KF_perf_figure(DB, Perf_6, x_label, y_label, self.color, self.bar_width)
        self.KF_perf_figure_line(DB, Perf_6, x_label, y_label, self.color)
